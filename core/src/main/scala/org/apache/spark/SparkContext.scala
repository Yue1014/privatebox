...
class SparkContext(config: SparkConf) extends Logging {
  ...
  val shuffleOpt: ShuffleOpt = ShuffleOpt.withName(config.get("spark.dft.shuffle", ShuffleOpt.WithoutOpt.string))
  ...
  def runJobSplit[T, U: ClassTag](rdd: RDD[T], func: Iterator[T] => U, num: Int): Vector[Array[U]] = {
    var v: Vector[Array[U]] = Vector()
    val per = rdd.partitions.length / num
    for (i <- 0 until num) {
      val r = runJob(rdd, func, i * per until (i + 1) * per)
      v = v ++ Vector(r)
    }
    v
  }
